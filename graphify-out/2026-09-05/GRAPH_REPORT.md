# Graph Report - STOCK MARKET TREND ANALYZER  (2026-09-04)

## Corpus Check
- 254 files · ~1,944,912 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2253 nodes · 2723 edges · 190 communities (132 shown, 42 thin omitted)
...
### Community 12 - "app.py"
Cohesion: 0.18
Nodes (23): backtest_endpoint(), contact(), dashboard(), disclaimer(), get_model_db_connection(), get_prediction(), get_sentiment(), get_stocks() (+15 more)
...
Cohesion: 0.18
Nodes (7): date, datetime, Builds a GDELT OR query from the primary company name and configured aliases,…, Determines whether a headline is genuinely relevant to the target company,…, Finds the earliest NSE trading day strictly after cal_date. Returns None if no…, Maps an article's IST timestamp to the correct NSE trading session date.…, Fetches GDELT articles for a UTC datetime range with recursive bisection…
...
### Community 125 - "delete_user_data"
Cohesion: 0.29
Nodes (9): delete_user_data(), get_db_connection(), get_user_data(), is_valid_email_format(), Interactive visual database viewer for all tables in users.db…, require_user_auth(), save_user_data(), view_database() (+1 more)
...
### Community 165 - "test_p0_readiness.py"
Cohesion: 0.42
...
### Community 167 - ".parse_gdelt_timestamp"
Cohesion: 0.33
Nodes (5): LowPrecisionTimestampError, Raised by parse_gdelt_timestamp() when a GDELT timestamp carries only date-…, Thread-safe increment of diagnostic counters., Parses a GDELT seendate string (UTC) into a 4-tuple: (source_timestamp,…, ValueError

### Community 169 - "apply_security_headers"
Cohesion: 0.33
Nodes (4): after_request, apply_security_headers(), handle_preflight_and_options(), before_request

### Community 176 - "probe_ambiguous_tickers.py"
Cohesion: 0.67
...
- **1035 isolated node(s):** `$schema`, `name`, `description`, `name`, `url` (+1030 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 1448 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **42 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `NewsFetcher` connect `NewsFetcher` to `TestAxisBankMatcherValidation`, `TestNewsFetcher`, `TestBigQueryGKGExtractor`, `generate_sentiment.py`, `TestKotakBankMatcherValidation`, `TestBajajFinanceMatcherValidation`, `TestBhartiAirtelMatcherValidation`, `TestICICIBankMatcherValidation`, `.fetch_gdelt_window`, `test_news_fetcher.py`, `process_ticker_news_fetch`, `cache.py`, `._deduplicate_articles`, `probe_live_gdelt.py`, `.parse_gdelt_timestamp`, `news_fetcher.py`, `._article_dedupe_key`, `probe_ambiguous_tickers.py`, `._get_session`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `TestNewsFetcher` connect `TestNewsFetcher` to `patch`, `NewsFetcher`, `test_news_fetcher.py`, `_make_article`, `process_ticker_news_fetch`, `GDELTRateLimiter`, `_gdelt_item`, `.test_D2b_itc_lifecycle_and_transition_boundary_tests`, `.parse_gdelt_timestamp`, `._article_dedupe_key`, `.test_mukesh_ambani_policy_corporate_positives_and_listicle_negatives`, `.test_A5_weekend`, `.test_A6_nse_holiday`, `.test_A7b_article_on_day_after_last_calendar_day`, `.test_A7_beyond_final_trading_date_returns_none`, `.test_sunday_news_maps_to_next_monday`, `.test_A1_trading_day_before_1530`, `.test_tcs_matching`, `.test_A2_trading_day_exactly_at_1530`?**
  _High betweenness centrality (0.040) - this node is a cross-community bridge._
- **Why does `download_macro_data()` connect `main.py` to `.parse_gdelt_timestamp`?**
...
- **Are the 6 inferred relationships involving `patch` (e.g. with `.test_07_real_rate_limiter_exhaustion_returns_json_429()` and `.test_08_http_500_sanitization_no_leaks()`) actually correct?**
